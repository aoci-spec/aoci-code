import { Warranty, WarrantyStatus, compareWarranty, touchWarranty, validateWarranty } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface WarrantyPage {
  readonly items: readonly Warranty[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for warranty claim records, keyed by tenant and id. */
export class WarrantyRepository {
  private readonly byTenant = new Map<string, Map<string, Warranty>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Warranty> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Warranty>();
  ...
  save(value: Warranty): Warranty {
    ...
    const stored = touchWarranty(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Warranty | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Warranty | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Warranty {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`warranty ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: WarrantyStatus): Warranty[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Warranty[] {
  ...
  all(tenantId: string): Warranty[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareWarranty);
  }
