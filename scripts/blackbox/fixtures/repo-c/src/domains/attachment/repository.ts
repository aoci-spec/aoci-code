import { Attachment, AttachmentStatus, compareAttachment, touchAttachment, validateAttachment } from "./model";
...
/** In-memory store for file attachment records, keyed by tenant and id. */
export class AttachmentRepository {
  private readonly byTenant = new Map<string, Map<string, Attachment>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Attachment> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Attachment>();
  ...
  save(value: Attachment): Attachment {
    validateAttachment(value);
    const tenant = this.tenantMap(value.tenantId);
    ...
    const stored = touchAttachment(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Attachment | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Attachment | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Attachment {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`attachment ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: AttachmentStatus): Attachment[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Attachment[] {
  ...
  all(tenantId: string): Attachment[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareAttachment);
  }
