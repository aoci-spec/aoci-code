...
When a task has an explicit code-change plan, first express that plan as a separate complete Code target in `aoci.code.target.txt`, run `aoci cognition plan diff --target-index aoci.code.target.txt`, inspect the complete current-to-target comparison, and only then edit business code. Keep `aoci.code.txt` as the current formal Index; do not use it as the planning file. Do not create a target file merely for an unplanned, purely local small fix.
