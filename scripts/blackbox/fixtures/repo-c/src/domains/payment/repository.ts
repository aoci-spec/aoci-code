import { Payment, PaymentStatus, comparePayment, touchPayment, validatePayment } from "./model";
...
/** In-memory store for payment attempt records, keyed by tenant and id. */
export class PaymentRepository {
  private readonly byTenant = new Map<string, Map<string, Payment>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Payment> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Payment>();
  ...
  save(value: Payment): Payment {
    ...
    const stored = touchPayment(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Payment | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Payment | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Payment {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`payment ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: PaymentStatus): Payment[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Payment[] {
  ...
  all(tenantId: string): Payment[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(comparePayment);
  }
