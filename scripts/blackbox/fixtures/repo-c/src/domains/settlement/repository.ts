import { Settlement, SettlementStatus, compareSettlement, touchSettlement, validateSettlement } from "./model";
...
/** In-memory store for settlement run records, keyed by tenant and id. */
export class SettlementRepository {
  private readonly byTenant = new Map<string, Map<string, Settlement>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Settlement> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Settlement>();
  ...
  save(value: Settlement): Settlement {
    validateSettlement(value);
    const tenant = this.tenantMap(value.tenantId);
    ...
    const stored = touchSettlement(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Settlement | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Settlement | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Settlement {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`settlement ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: SettlementStatus): Settlement[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Settlement[] {
  ...
  all(tenantId: string): Settlement[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareSettlement);
  }
