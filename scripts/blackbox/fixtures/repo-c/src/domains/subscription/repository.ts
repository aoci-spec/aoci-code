import { Subscription, SubscriptionStatus, compareSubscription, touchSubscription, validateSubscription } from "./model";
...
/** In-memory store for recurring subscription records, keyed by tenant and id. */
export class SubscriptionRepository {
  private readonly byTenant = new Map<string, Map<string, Subscription>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Subscription> {
  ...
  save(value: Subscription): Subscription {
    validateSubscription(value);
    const tenant = this.tenantMap(value.tenantId);
    ...
    const stored = touchSubscription(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Subscription | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Subscription | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Subscription {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`subscription ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: SubscriptionStatus): Subscription[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Subscription[] {
  ...
  all(tenantId: string): Subscription[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareSubscription);
  }
