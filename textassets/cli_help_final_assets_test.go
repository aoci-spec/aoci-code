...
func TestCLIHelpFinalAssetsKeepExactContent(
	...
	tests := []struct {
		...
		},
		{
			...
			expected: "不提供单条参数时，固定读取仓库根的 aoci.code.target.txt，绑定最终源码哈希并通过共享原子批次更新正式 Code 索引；成功后用正式 Code 同步 target，全程不调用模型。\n" +
				"提供 --path 后进入单条模式：对指定文件写入完整单行条目，已有条目整行替换，无则插入所属目录段。\n" +
