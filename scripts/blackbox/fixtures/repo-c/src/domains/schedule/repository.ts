import { Schedule, ScheduleStatus, compareSchedule, touchSchedule, validateSchedule } from "./model";
...
/** In-memory store for scheduled run records, keyed by tenant and id. */
export class ScheduleRepository {
  private readonly byTenant = new Map<string, Map<string, Schedule>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Schedule> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Schedule>();
  ...
  save(value: Schedule): Schedule {
    ...
    const stored = touchSchedule(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Schedule | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Schedule | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Schedule {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`schedule ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: ScheduleStatus): Schedule[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Schedule[] {
  ...
  all(tenantId: string): Schedule[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareSchedule);
  }
