import { Variant, VariantStatus, compareVariant, touchVariant, validateVariant } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface VariantPage {
  readonly items: readonly Variant[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for product variant records, keyed by tenant and id. */
export class VariantRepository {
  private readonly byTenant = new Map<string, Map<string, Variant>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Variant> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Variant>();
  ...
  save(value: Variant): Variant {
    ...
    const stored = touchVariant(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Variant | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Variant | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Variant {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`variant ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: VariantStatus): Variant[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Variant[] {
  ...
  all(tenantId: string): Variant[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareVariant);
  }
