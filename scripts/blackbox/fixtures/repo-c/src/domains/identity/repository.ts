import { Identity, IdentityStatus, compareIdentity, touchIdentity, validateIdentity } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface IdentityPage {
  readonly items: readonly Identity[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for identity record records, keyed by tenant and id. */
export class IdentityRepository {
  private readonly byTenant = new Map<string, Map<string, Identity>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Identity> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Identity>();
  ...
  insert(value: Identity): Identity {
    ...
    if (tenant.has(value.id)) {
      throw new ConflictError(`identity ${value.id} already exists`);
    }
    const referenceKey = this.referenceKey(value.tenantId, value.reference);
    if (this.byReference.has(referenceKey)) {
      throw new ConflictError(`identity reference ${value.reference} is already used`);
    }
  ...
  save(value: Identity): Identity {
    ...
    const stored = touchIdentity(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Identity | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Identity | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Identity {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`identity ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: IdentityStatus): Identity[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Identity[] {
  ...
  all(tenantId: string): Identity[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareIdentity);
  }
