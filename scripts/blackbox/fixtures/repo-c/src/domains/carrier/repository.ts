import { Carrier, CarrierStatus, compareCarrier, touchCarrier, validateCarrier } from "./model";
...
/** In-memory store for delivery carrier records, keyed by tenant and id. */
export class CarrierRepository {
  private readonly byTenant = new Map<string, Map<string, Carrier>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Carrier> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Carrier>();
  ...
  save(value: Carrier): Carrier {
    ...
    const stored = touchCarrier(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Carrier | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Carrier | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Carrier {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`carrier ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: CarrierStatus): Carrier[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Carrier[] {
  ...
  all(tenantId: string): Carrier[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareCarrier);
  }
