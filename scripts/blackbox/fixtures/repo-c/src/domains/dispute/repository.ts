import { Dispute, DisputeStatus, compareDispute, touchDispute, validateDispute } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface DisputePage {
  readonly items: readonly Dispute[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for payment dispute records, keyed by tenant and id. */
export class DisputeRepository {
  private readonly byTenant = new Map<string, Map<string, Dispute>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Dispute> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Dispute>();
  ...
  insert(value: Dispute): Dispute {
    validateDispute(value);
    const tenant = this.tenantMap(value.tenantId);
    if (tenant.has(value.id)) {
      throw new ConflictError(`dispute ${value.id} already exists`);
    }
    const referenceKey = this.referenceKey(value.tenantId, value.reference);
    if (this.byReference.has(referenceKey)) {
      throw new ConflictError(`dispute reference ${value.reference} is already used`);
    }
  ...
  save(value: Dispute): Dispute {
    ...
    const stored = touchDispute(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Dispute | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Dispute | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Dispute {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`dispute ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: DisputeStatus): Dispute[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Dispute[] {
  ...
  all(tenantId: string): Dispute[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareDispute);
  }
