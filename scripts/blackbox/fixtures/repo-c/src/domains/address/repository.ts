import { Address, AddressStatus, compareAddress, touchAddress, validateAddress } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface AddressPage {
  readonly items: readonly Address[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for postal address records, keyed by tenant and id. */
export class AddressRepository {
  private readonly byTenant = new Map<string, Map<string, Address>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Address> {
  ...
  insert(value: Address): Address {
    ...
    if (tenant.has(value.id)) {
      throw new ConflictError(`address ${value.id} already exists`);
    }
    const referenceKey = this.referenceKey(value.tenantId, value.reference);
    if (this.byReference.has(referenceKey)) {
      throw new ConflictError(`address reference ${value.reference} is already used`);
    }
  ...
  save(value: Address): Address {
    ...
    const stored = touchAddress(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Address | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Address | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Address {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`address ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: AddressStatus): Address[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Address[] {
  ...
  all(tenantId: string): Address[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareAddress);
  }
