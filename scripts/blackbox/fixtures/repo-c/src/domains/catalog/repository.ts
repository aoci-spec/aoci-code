import { Catalog, CatalogStatus, compareCatalog, touchCatalog, validateCatalog } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface CatalogPage {
  readonly items: readonly Catalog[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for product catalog records, keyed by tenant and id. */
export class CatalogRepository {
  private readonly byTenant = new Map<string, Map<string, Catalog>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Catalog> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Catalog>();
  ...
  save(value: Catalog): Catalog {
    ...
    const stored = touchCatalog(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Catalog | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Catalog | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Catalog {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`catalog ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: CatalogStatus): Catalog[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Catalog[] {
  ...
  all(tenantId: string): Catalog[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareCatalog);
  }
