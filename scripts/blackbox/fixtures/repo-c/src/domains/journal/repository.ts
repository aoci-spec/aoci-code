import { Journal, JournalStatus, compareJournal, touchJournal, validateJournal } from "./model";
...
/** In-memory store for journal batch records, keyed by tenant and id. */
export class JournalRepository {
  private readonly byTenant = new Map<string, Map<string, Journal>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Journal> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Journal>();
  ...
  save(value: Journal): Journal {
    ...
    const stored = touchJournal(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Journal | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Journal | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Journal {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`journal ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: JournalStatus): Journal[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Journal[] {
  ...
  all(tenantId: string): Journal[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareJournal);
  }
