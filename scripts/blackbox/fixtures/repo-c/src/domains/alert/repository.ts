import { Alert, AlertStatus, compareAlert, touchAlert, validateAlert } from "./model";
...
/** In-memory store for operational alert records, keyed by tenant and id. */
export class AlertRepository {
  private readonly byTenant = new Map<string, Map<string, Alert>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Alert> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Alert>();
  ...
  save(value: Alert): Alert {
    ...
    const stored = touchAlert(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Alert | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Alert | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Alert {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`alert ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: AlertStatus): Alert[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Alert[] {
  ...
  all(tenantId: string): Alert[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareAlert);
  }
