...
1.2 **一键理解已有系统，直接接手开发。** 对 50 万行以内的现有系统，一句话让 Agent 建立整体认知；它会报告自己对系统的掌握程度，然后无缝接手开发。可支持的规模由索引大小决定，而不是代码行数本身：实测已支持 70 万行商业系统的开发，对应索引约 300K token。

1.3 **换人、换 Agent、换对话，认知不丢。** 索引随代码保存在仓库里，由 Git 版本化。项目更换项目经理或开发者、切换到另一个 Agent、新开一个对话，读一次索引即可立刻接上，跨 Agent、跨对话无缝衔接。

1.4 **首次索引建立后自动维护。** MCP 自动发现代码变化并签发需要更新的条目，Agent 在完成开发任务时顺手补齐，索引始终与当前代码版本一致，开发过程无感。

## 2. 📌 使用注意事项

2.1 **首次建立索引需要时间。** Agent 要读完全部受管文件并逐个撰写条目，系统越大耗时越长：20 万行代码大约需要 1 小时，具体取决于所用模型和 Agent 的速度。索引按批次建立，中途中断后可以继续。

2.2 **有数据库的系统，建议再建数据库索引。** 目前支持 MySQL、PostgreSQL，以及受限支持的 openGauss 6.0.5。先建代码索引，再建数据库索引，代码认知与表级认知一起交付时，Agent 对系统的理解更完整。

2.3 **只读你的系统，不联网，不存凭据。** AOCI-CODE 只读取源码和数据库的表结构，不读业务数据，只在项目目录内写入索引文件和自身状态。它本身不联网，不上传任何内容，数据库凭据只通过环境变量名引用，从不保存。索引内容由你自己的 Agent 在本地撰写，走的是你已经在用的模型通道，AOCI-CODE 不引入任何新的数据出口。
...
请从 canonical source 获取 AOCI-CODE，或使用 GitHub Releases 提供的签名包。使用预构建二进制前，请按[安装指南](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/install.md#signed-github-release-packages)选择基础、推荐或完整校验层级，并准确说明已完成的层级。请把本 README 和已验证二进制的稳定绝对路径交给 Codex、Claude Code、Cursor、OpenCode 等受信任 AI Agent。AI Agent 可以按照项目内的说明运行初始化、完成 MCP 接入并建立第一份索引。
...
签名包路线和可执行验证命令见[安装指南](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/install.md#signed-github-release-packages)；下面仍保留从源码构建的路线。

### 1. 📦 当前 RC：使用经验证的软件包或从源码构建

> [!IMPORTANT]
> AOCI-CODE v0.1.0-rc11 是当前发布候选版本。它是采用 FSL-1.1-MIT 的 Fair Source/source-available 软件；具体条款见 [LICENSE](LICENSE)。可以从 canonical source 构建，也可以使用 [v0.1.0-rc11 GitHub Release](https://github.com/aoci-spec/aoci-code/releases/tag/v0.1.0-rc11) 提供的签名包；使用前请遵循[发布验证流程](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/install.md#signed-github-release-packages)。

签名 Release 二进制报告 `aoci version 0.1.0-rc11`。源码构建则报告由精确
Git checkout 派生的版本，例如 Release tag 之后的
`v0.1.0-rc11-1-g<short-commit>`；工作树不干净时还会带 `-dirty`，并单独报告
...
如需匿名下载，请在浏览器中打开
[v0.1.0-rc11 Release 页面](https://github.com/aoci-spec/aoci-code/releases/tag/v0.1.0-rc11)，
并下载所需 archive 与验证资产。
...
AOCI 在 `aoci_overview` 上声明了单工具的结果大小上限。有些宿主会把过大的工具结果落盘、只把预览放进模型上下文 —— 那对认知交付等于静默失败 —— 这个声明让它们只为这一个工具抬高阈值。在 Claude Code 上，`24000` 因此可以放心使用，480 个对象的索引由 5 块变成 3 块。这是静态声明，不做任何宿主探测；不认识它的宿主会按 MCP 规范忽略。
...
| 主题 | 文档 |
| --- | --- |
| 第一次使用 | [Getting Started](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/getting-started.md) |
| 安装、升级与回滚 | [Install](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/install.md) · [Upgrade](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/upgrading.md) · [Rollback](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/rollback.md) · [Uninstall](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/uninstall.md) |
| AI Agent 与宿主 | [Agent Integrations](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/agent-integrations.md) · [Windows Host Agent](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/windows-host-agent.md) |
| Whole-Index 与刷新 | [Overview Delivery](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/overview-delivery.md) · [Cognition Refresh](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/cognition-refresh.md) |
...
| 生命周期 | [Getting Started](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/getting-started.md) · [Upgrade](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/upgrading.md) · [Cognition Refresh](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/cognition-refresh.md) |
| 格式与协议 | [Index Format](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-index-format-v1.txt) · [Cognition Volumes Spec](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-cognition-volumes-v1.txt) · [Object FRAS v2](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-object-fras-v2.txt) |
| 研究与发布 | [Supply Chain](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/supply-chain.md) |

> 上述文档和公开合同链接固定到 `v0.1.0-rc11`，因此即使从不包含 `docs/` 和
...
外部贡献者可以按照 [CONTRIBUTING.md](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/CONTRIBUTING.md) 提交聚焦的改进。贡献者必须有权提交相关工作；被接受的贡献受仓库许可证和已发布的 inbound terms 约束，维护者也可能在合并前要求补充贡献者文件。

请勿在公开 Issue 中披露疑似漏洞。请遵循 [SECURITY.md](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/SECURITY.md)；受监控的私密报告渠道和明确响应责任仍是公开 Release 的必要条件。

AOCI-CODE v0.1.0-rc11 是采用 FSL-1.1-MIT 的 Fair Source/source-available 软件。适用条款见 [LICENSE](LICENSE)。
