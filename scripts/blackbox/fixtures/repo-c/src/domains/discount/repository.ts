import { Discount, DiscountStatus, compareDiscount, touchDiscount, validateDiscount } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface DiscountPage {
  readonly items: readonly Discount[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for discount rule records, keyed by tenant and id. */
export class DiscountRepository {
  private readonly byTenant = new Map<string, Map<string, Discount>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Discount> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Discount>();
  ...
  save(value: Discount): Discount {
    ...
    const stored = touchDiscount(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Discount | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Discount | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Discount {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`discount ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: DiscountStatus): Discount[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Discount[] {
  ...
  all(tenantId: string): Discount[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareDiscount);
  }
