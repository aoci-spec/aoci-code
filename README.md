...
![Status](https://img.shields.io/badge/status-v0.1.0--rc11-orange)
...
## 1. 🎯 What AOCI-CODE does

1.1 **Keep developing a system inside a Code Agent.** In Codex, Claude Code, Cursor, OpenCode, and similar Agents, grow a system from nothing into a complete 500,000-line codebase plus its database. The Agent always works with the whole-system cognition the AOCI index provides instead of searching and re-reading the entire codebase for every task. Non-professional developers can keep iterating on their own systems; professional developers can hand the whole system to an Agent for collaborative evolution and keep their attention on architecture and design decisions.

1.2 **Understand an existing system in one step and take over its development.** For an existing system of up to 500,000 lines, one sentence has the Agent build whole-system cognition; it reports how well it has mastered the system, then takes over development seamlessly. The supported scale is set by the index size, not by the line count as such: in practice a 700,000-line commercial system is developed this way today, with an index of about 300K tokens.

1.3 **Change the people, the Agent, or the conversation without losing cognition.** The index lives in the repository next to the code and is versioned by Git. When a project changes its project manager or developers, switches to another Agent, or opens a new conversation, one read of the index picks up where things left off, seamlessly across Agents and conversations.

1.4 **After the first index, maintenance is automatic.** The MCP detects code changes and issues the Entries that need updating; the Agent fills them in as it finishes each development task, so the index always matches the current code version and development is never interrupted.

## 2. 📌 Notes before you start

2.1 **The first index takes time.** The Agent reads every managed file and writes one Entry per file, so a larger system takes longer: about 1 hour for 200,000 lines of code, depending on the model and the Agent's speed. The index is built in batches, and an interrupted build resumes where it stopped.
...
The signed-package route and executable verification commands are in the [installation guide](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/install.md#signed-github-release-packages). The source-build route remains available below.

### 1. 📦 Current RC: use a verified package or build from source

> [!IMPORTANT]
> AOCI-CODE v0.1.0-rc11 is the current release candidate. It is Fair Source/source-available software under FSL-1.1-MIT; see [LICENSE](LICENSE). Build from canonical source or use a signed package from the [v0.1.0-rc11 GitHub Release](https://github.com/aoci-spec/aoci-code/releases/tag/v0.1.0-rc11) after following the [release verification procedure](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/install.md#signed-github-release-packages).

The signed Release binary identifies itself as `aoci version 0.1.0-rc11`. A
source build identifies the exact checkout instead and may report a development
version such as `v0.1.0-rc11-1-g<short-commit>` (plus `-dirty` when applicable),
...
```bash
gh auth login
gh release download v0.1.0-rc11 --repo aoci-spec/aoci-code
```

For an anonymous download, use the
[v0.1.0-rc11 Release page](https://github.com/aoci-spec/aoci-code/releases/tag/v0.1.0-rc11)
...
AOCI declares a per-tool result-size allowance on `aoci_overview`, so a Host that would otherwise persist a large tool result to disk — putting only a preview in the model's context, which for cognition delivery is a silent failure — raises that threshold for this tool alone. On Claude Code this makes the full `24000` usable, delivering a 480-object index in three chunks instead of five. Nothing is probed: the declaration is static, and a Host that does not recognize it ignores it, as the MCP specification requires.
...
| Topic | Document |
| --- | --- |
| First use | [Getting Started](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/getting-started.md) |
| Installation, upgrade, and rollback | [Install](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/install.md) · [Upgrade](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/upgrading.md) · [Rollback](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/rollback.md) · [Uninstall](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/uninstall.md) |
| AI Agents and hosts | [Agent Integrations](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/agent-integrations.md) · [Windows Host Agent](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/windows-host-agent.en.md) |
| Whole-Index and refresh | [Overview Delivery](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/overview-delivery.md) · [Cognition Refresh](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/cognition-refresh.md) |
| Cognition Volumes | [Volumes](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/cognition-volumes.md) · [Volumes Contract](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-cognition-volumes-v1.txt) |
| System Cognition | [System Cognition Runtime Contract](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-system-cognition-runtime-v1.txt) |
| Managed Scope | [Managed Scope and Budget](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/managed-scope-and-budget.md) · [Safe Inventory](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/safe-inventory-and-scope-refresh.md) |
| Database | [Database Evidence](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/database-evidence.md) · [Database Cognition Authoring](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/database-cognition-authoring.md) |
| Lifecycle | [Getting Started](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/getting-started.md) · [Upgrade](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/upgrading.md) · [Cognition Refresh](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/cognition-refresh.md) |
| Formats and protocols | [Index Format](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-index-format-v1.txt) · [Cognition Volumes Spec](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-cognition-volumes-v1.txt) · [Object FRAS v2](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/spec/public/aoci-object-fras-v2.txt) |
| Research and release | [Supply Chain](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/docs/supply-chain.md) |

> Documentation and public-contract links are pinned to `v0.1.0-rc11` so they
...
- **Fault-injection scenarios** — 56 scenarios covering cursor tampering,
...
Focused external contributions may be submitted through the process in [CONTRIBUTING.md](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/CONTRIBUTING.md). Contributors must have the right to submit their work; accepted contributions are governed by the repository license and any published inbound terms, and maintainers may require additional contributor documentation before merging.

Do not disclose suspected vulnerabilities in public Issues. Follow [SECURITY.md](https://github.com/aoci-spec/aoci-code/blob/v0.1.0-rc11/SECURITY.md); a monitored private reporting channel and clear response ownership remain prerequisites for a public Release.

AOCI-CODE v0.1.0-rc11 is Fair Source/source-available software licensed under FSL-1.1-MIT. See [LICENSE](LICENSE) for the governing terms.
