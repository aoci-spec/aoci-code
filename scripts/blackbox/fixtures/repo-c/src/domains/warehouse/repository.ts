import { Warehouse, WarehouseStatus, compareWarehouse, touchWarehouse, validateWarehouse } from "./model";
...
/** In-memory store for storage facility records, keyed by tenant and id. */
export class WarehouseRepository {
  private readonly byTenant = new Map<string, Map<string, Warehouse>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Warehouse> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Warehouse>();
  ...
  save(value: Warehouse): Warehouse {
    validateWarehouse(value);
    const tenant = this.tenantMap(value.tenantId);
    ...
    const stored = touchWarehouse(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Warehouse | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Warehouse | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Warehouse {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`warehouse ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: WarehouseStatus): Warehouse[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Warehouse[] {
  ...
  all(tenantId: string): Warehouse[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareWarehouse);
  }
