import { Document, DocumentStatus, compareDocument, touchDocument, validateDocument } from "./model";
...
/** In-memory store for stored document records, keyed by tenant and id. */
export class DocumentRepository {
  private readonly byTenant = new Map<string, Map<string, Document>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Document> {
  ...
  save(value: Document): Document {
    ...
    const stored = touchDocument(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Document | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Document | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Document {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`document ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: DocumentStatus): Document[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Document[] {
  ...
  all(tenantId: string): Document[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareDocument);
  }
