import { Role, RoleStatus, compareRole, touchRole, validateRole } from "./model";
...
/** In-memory store for authorization role records, keyed by tenant and id. */
export class RoleRepository {
  private readonly byTenant = new Map<string, Map<string, Role>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Role> {
  ...
  insert(value: Role): Role {
    ...
    if (tenant.has(value.id)) {
      throw new ConflictError(`role ${value.id} already exists`);
    }
    const referenceKey = this.referenceKey(value.tenantId, value.reference);
    if (this.byReference.has(referenceKey)) {
      throw new ConflictError(`role reference ${value.reference} is already used`);
    }
  ...
  save(value: Role): Role {
    ...
    const stored = touchRole(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Role | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Role | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Role {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`role ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: RoleStatus): Role[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Role[] {
  ...
  all(tenantId: string): Role[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareRole);
  }
