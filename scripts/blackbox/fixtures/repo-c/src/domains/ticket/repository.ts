import { Ticket, TicketStatus, compareTicket, touchTicket, validateTicket } from "./model";
...
/** In-memory store for support ticket records, keyed by tenant and id. */
export class TicketRepository {
  private readonly byTenant = new Map<string, Map<string, Ticket>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Ticket> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Ticket>();
  ...
  save(value: Ticket): Ticket {
    ...
    const stored = touchTicket(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Ticket | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Ticket | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Ticket {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`ticket ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: TicketStatus): Ticket[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Ticket[] {
  ...
  all(tenantId: string): Ticket[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareTicket);
  }
