...
const response = root => ({status: 200, headers: {get: () => root}, json: async () => ({root})});
...
(async () => {
  ...
  // Even within one repository, an older JSON body may finish last.
  {
    ...
    context.fetch = async () => ({status: 200, headers: {get: () => 'old'}, json: () => { parsing.resolve(); return body.promise; }});
    const pending = ui.refresh(true);
