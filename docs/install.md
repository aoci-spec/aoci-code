...
The signed Release binary reports `aoci version 0.1.0-rc6`. A source build
reports the version derived from its exact Git checkout, such as
`v0.1.0-rc6-1-g<short-commit>` after the Release tag, and reports that Git
...
[v0.1.0-rc6 GitHub Release](https://github.com/aoci-spec/aoci-code/releases/tag/v0.1.0-rc6). The
`v0.1.0-rc6` release has exactly 16 uploaded assets:
...
```bash
ARCHIVE=aoci_0.1.0-rc6_linux_amd64.tar.gz # choose the actual platform asset
...
```powershell
$Archive = "aoci_0.1.0-rc6_windows_amd64.zip" # choose the actual architecture
