import { Currency, CurrencyStatus, compareCurrency, touchCurrency, validateCurrency } from "./model";
...
/** In-memory store for currency rate records, keyed by tenant and id. */
export class CurrencyRepository {
  private readonly byTenant = new Map<string, Map<string, Currency>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Currency> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Currency>();
  ...
  insert(value: Currency): Currency {
    ...
    if (tenant.has(value.id)) {
      throw new ConflictError(`currency ${value.id} already exists`);
    }
    const referenceKey = this.referenceKey(value.tenantId, value.reference);
    if (this.byReference.has(referenceKey)) {
      throw new ConflictError(`currency reference ${value.reference} is already used`);
    }
  ...
  save(value: Currency): Currency {
    ...
    const stored = touchCurrency(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Currency | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Currency | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Currency {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`currency ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: CurrencyStatus): Currency[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Currency[] {
  ...
  all(tenantId: string): Currency[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareCurrency);
  }
