...
### AOCI-first source reading

Before starting a task, ensure that AOCI cognition is complete, current, and reliable. If the formal AOCI assets are absent, incomplete, Dirty, Stale, governance-unaligned, or in Recovery, do not substitute broader source reading for system cognition; read `aoci_rules`, enter the current Guide, and follow its exact path to complete, repair, and align AOCI. If formal AOCI is Current and governance-aligned but the model lost cognition because of a new Run or context compaction, reload `aoci_rules` and one ordinary complete `aoci_overview`; do not modify the Index.

Once reliable cognition is established, use AOCI global cognition by default as the repository-level system context when answering user questions, analyzing or fixing bugs, and designing or implementing changes. Read only the small set of source, tests, configuration, and contracts directly needed for the current question, solution, implementation, and verification. Expand that evidence set only when a specific missing fact could change the answer, solution, or verification result, and read only the minimum objects that can resolve it. Stop reading as soon as the evidence is sufficient. AOCI does not replace direct source evidence for changed objects and verified behavior.

### Code-change planning

For every new feature, bug fix, or other task with an explicit code-change plan, first express the intended final state as a separate complete Code target in `aoci.code.target.txt`, run `aoci cognition plan diff --target-index aoci.code.target.txt`, inspect the complete current-to-target comparison, and only then edit business code. The conventional root target file is a generated planning artifact outside Managed Scope. Keep `aoci.code.txt` as the current formal Index; do not use it as the planning file. After implementation and verification, complete ordinary cognition maintenance, then copy each resulting formal Entry from `aoci.code.txt` over its corresponding target Entry so implemented objects converge back to the formal Index. Do not create a target file merely for an unplanned, purely local small edit that is neither a feature nor a bug fix.
