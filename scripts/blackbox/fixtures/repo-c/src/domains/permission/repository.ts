import { Permission, PermissionStatus, comparePermission, touchPermission, validatePermission } from "./model";
...
/** In-memory store for permission grant records, keyed by tenant and id. */
export class PermissionRepository {
  private readonly byTenant = new Map<string, Map<string, Permission>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Permission> {
  ...
  save(value: Permission): Permission {
    validatePermission(value);
    const tenant = this.tenantMap(value.tenantId);
    ...
    const stored = touchPermission(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Permission | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Permission | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Permission {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`permission ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: PermissionStatus): Permission[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Permission[] {
  ...
  all(tenantId: string): Permission[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(comparePermission);
  }
