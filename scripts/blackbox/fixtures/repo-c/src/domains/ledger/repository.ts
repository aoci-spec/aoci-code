import { Ledger, LedgerStatus, compareLedger, touchLedger, validateLedger } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface LedgerPage {
  readonly items: readonly Ledger[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for accounting ledger entry records, keyed by tenant and id. */
export class LedgerRepository {
  private readonly byTenant = new Map<string, Map<string, Ledger>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Ledger> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Ledger>();
  ...
  save(value: Ledger): Ledger {
    ...
    const stored = touchLedger(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Ledger | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Ledger | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Ledger {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`ledger ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: LedgerStatus): Ledger[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Ledger[] {
  ...
  all(tenantId: string): Ledger[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(compareLedger);
  }
