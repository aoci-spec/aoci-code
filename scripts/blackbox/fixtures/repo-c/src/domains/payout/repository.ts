import { Payout, PayoutStatus, comparePayout, touchPayout, validatePayout } from "./model";
import { ConflictError, NotFoundError } from "../../infra/errors";

export interface PayoutPage {
  readonly items: readonly Payout[];
  readonly total: number;
  readonly offset: number;
}

/** In-memory store for merchant payout records, keyed by tenant and id. */
export class PayoutRepository {
  private readonly byTenant = new Map<string, Map<string, Payout>>();
  private readonly byReference = new Map<string, string>();

  private tenantMap(tenantId: string): Map<string, Payout> {
    const existing = this.byTenant.get(tenantId);
    if (existing) {
      return existing;
    }
    const created = new Map<string, Payout>();
  ...
  save(value: Payout): Payout {
    ...
    const stored = touchPayout(value);
    tenant.set(stored.id, stored);
    return stored;
  }

  find(tenantId: string, id: string): Payout | undefined {
    return this.byTenant.get(tenantId)?.get(id);
  }

  findByReference(tenantId: string, reference: string): Payout | undefined {
    const id = this.byReference.get(this.referenceKey(tenantId, reference));
    return id === undefined ? undefined : this.find(tenantId, id);
  }

  require(tenantId: string, id: string): Payout {
    const found = this.find(tenantId, id);
    if (!found) {
      throw new NotFoundError(`payout ${id} not found for tenant ${tenantId}`);
    }
    return found;
  }

  listByStatus(tenantId: string, status: PayoutStatus): Payout[] {
    return this.all(tenantId).filter((item) => item.status === status);
  }

  listByLabel(tenantId: string, label: string): Payout[] {
  ...
  all(tenantId: string): Payout[] {
    return [...(this.byTenant.get(tenantId)?.values() ?? [])].sort(comparePayout);
  }
